...
const isSavingToMysql = ref(false)
const saveStatus = ref('')
const saveError = ref('')
const storageDriver = computed(() => runtimeConfig.public.cvStorageDriver)
const mysqlEnabled = computed(() => storageDriver.value === 'mysql')
...
async function persistSettings(mode: 'auto' | 'manual') {
  ...
  try {
    ...
    saveStatus.value = mysqlEnabled.value
      ? 'Saved to MySQL.'
    ...
    saveError.value = error instanceof Error
      ? error.message
      : 'Save failed. Check MySQL access and try again.'
  }
  finally {
    if (mode === 'manual') {
      isSavingToMysql.value = false
    }
  }
}
</script>
  ...
  <div class="settings">
    ...
    >
      <div class="form__section px-6 py-3">
        ...
        <button
          class="form__btn form__btn--mysql"
          type="button"
          :disabled="isSavingToMysql"
          @click="persistSettings('manual')"
        >
          {{ isSavingToMysql ? 'Saving...' : 'Save to MySQL' }}
        </button>
        <p class="form__note">
          Storage mode: <strong>{{ mysqlEnabled ? 'MySQL' : 'Local browser storage' }}</strong>
        </p>
        <p
          v-if="saveStatus"
          class="form__note form__note--success"
        >
          {{ saveStatus }}
        </p>
        <p
          v-if="saveError"
          class="form__note form__note--error"
        >
          {{ saveError }}
        </p>
      </div>
